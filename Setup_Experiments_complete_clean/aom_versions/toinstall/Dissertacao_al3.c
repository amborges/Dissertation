...
int canISlip(int mi_row, int mi_col, BLOCK_SIZE depth, int original){
    //TRUE se pode descer na árvore de particionamento, FALSE se não
    if(!WASLOADCSV){
    	printf("CSV doesnt load\n");
    }
    
    ACTUALDEPTHBLOCK = MBTD[mi_col % 32][mi_col % 32];
    
    //return original; //no effect
    
    //if (depth > ACTUALDEPTHBLOCK) return 1; //EQUAL
    //else if (depth == ACTUALDEPTHBLOCK) return original;
    if (depth >= ACTUALDEPTHBLOCK) return 1; //EQUAL + UNDER
    else return 0;
}

int fullsearch(BLOCK_SIZE bs){
    
    if(stopLoop >= 5) return 0; //Previne loops infinitos
    
    if(bs > ACTUALDEPTHBLOCK + 3) return 1; //limita a um modo acima de intra/inter // EQUAL + ABOVE
    if(bs < ACTUALDEPTHBLOCK - 3) return 1; //limita a um modo acima de intra/inter //
    //if(bs > ACTUALDEPTHBLOCK) return 1; //limita a um unico modo de intra/inter //EQUAL
    
    return 0; //permite buscar tudo
}
